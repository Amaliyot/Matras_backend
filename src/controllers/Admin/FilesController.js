...
module.exports = class FilesController{
    ...
    static async AdressPhotosGetController(req, res, next){
        try {
    ...
    static async DeleteAdressPhotoPostController(req, res, next){
        try {
            const photo = await req.db.adress_photos.findOne({
                where: {
                    photo_id: req.params.id
                }
            })
            ...
            await req.db.adress_photos.destroy({
                where: {
                    photo_id: photo.photo_id
                }
            })
            fs.unlink(path.join(__dirname, '..', '..', 'public', 'files', 'adressPhotos', `${photo.photo_name + photo.photo_ext}`))
